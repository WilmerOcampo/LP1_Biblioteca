...
<!-- Inicio del menu -->
      ...
      <br>
		<h1 class="d-flex justify-content-center" style="text-transform: uppercase;"><strong>Lista de Alumnos</strong></h1>
    ...
    <div class="container">
        ...
        <table class="table table-striped">
            ...
            <tr>
                <th><%= temporal.getIdalumno() %></th>
                <td><%= temporal.getNombrealumno() %></td>
                <td><%= temporal.getApellidoalumno() %></td>
                <td><%= temporal.getDireccion() %></td>
                <td><%= temporal.getTelefono() %></td>
                <td><%= temporal.getEmail() %></td>
                <td><a class="btn btn-outline-success" href="ControladorAlumnos?instruccion=cargar&idalumno=<%= temporal.getIdalumno() %>">Editar</a>
                    <a class="btn btn-outline-danger" href="#" onclick="confirmarEliminacion(<%= temporal.getIdalumno() %>)">Eliminar</a>
                    <a class="btn btn-outline-danger" href="ControladorAlumnos?instruccion=eliminar&idalumno=<%= temporal.getIdalumno() %>">Eliminar</a>
                </td>
            </tr>
            <% } %>
        </table>
        ...
        <div class="d-flex justify-content-center">
            <input class="btn btn-primary" type="button" value="Crear Alumno" style="color: #fff; font-weight:600"
    ...
    <script>
	    function confirmarEliminacion(idalumno) {
	        if (confirm("¿Estás seguro de que deseas eliminar este registro?")) {
	            window.location.href = "ControladorAlumnos?instruccion=eliminar&idalumno=" + idalumno;
	        }
	    }
	</script>
